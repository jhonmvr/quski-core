...
public class ReportGenerator {
	...
	public ReportGenerator(Map<String, Object> map, byte file[], String reportName, String xpathSource)
	...
	public static void main(String[] args) {
		...
		try {
			...
			if (baos != null) {
				Path pathx = Paths.get(
						"C:\\wildfly-11.0.0.Final\\portalservicios_dir\\siniestro_agricola\\reporte\\testjona.pdf");
		...
		/*
		 * try { String tipoReporte="TICKETS"; String formato="PDF"; String
		 * fechaDesde="2016-04-01"; String fechaHasta="2016-04-31"; String pos=" ";
		 * log.info("=======> pos " + pos ); if( tipoReporte == null ||
		 * tipoReporte.equalsIgnoreCase("") || formato == null ||
		 * formato.equalsIgnoreCase("") ){ throw new
		 * ServletException("ERROR ReporteVentasAndTicketsServlet NO SE ENVIA PARAMETROS REQUERIDOS TIPO REPORTE Y FORMATO"
		 * ); } Properties prop =
		 * ManagePropertiesFile.loadProperties(Constantes.FILENAME_GLOBAL_PROPS); String
		 * path= prop.getProperty( Constantes.HOME_CONFIG_PROPS ) + prop.getProperty(
		 * Constantes.REPORT_PATH_PROPS ); Map<String, Object> parameters= new
		 * HashMap<String,Object>(); if( fechaDesde != null && fechaDesde.length()>= 10
		 * ){ fechaDesde = fechaDesde.substring(0,10); Calendar cal =
		 * Calendar.getInstance(); cal.setTime( UtilsGe.convertStringToDate(fechaDesde,
		 * Constantes.DATE_FORMAT_WEB) ); if( cal.get( Calendar.DAY_OF_MONTH )> 0 ){
		 * cal.set( Calendar.DAY_OF_MONTH , cal.get( Calendar.DAY_OF_MONTH )-1); }
		 * parameters.put("FECHA_DESDE", cal.getTime() ); } if( fechaHasta != null &&
		 * fechaHasta.length()>= 10 ){ fechaHasta = fechaHasta.substring(0,10); Calendar
		 * cal = Calendar.getInstance(); cal.setTime(
		 * UtilsGe.convertStringToDate(fechaHasta, Constantes.DATE_FORMAT_WEB) ); if(
		 * cal.get( Calendar.DAY_OF_MONTH )> 0 ){ cal.set( Calendar.DAY_OF_MONTH ,
		 * cal.get( Calendar.DAY_OF_MONTH )-1); } parameters.put("FECHA_HASTA",
		 * cal.getTime() ); } parameters.put("POS", pos); String reportPath=""; if(
		 * tipoReporte.equalsIgnoreCase( "TICKETS" ) ){ parameters.put("SUBREPORT_DIR",
		 * path); reportPath=path
		 * +prop.getProperty(Constantes.REPORT_TICKETS_GENERAL_PROPS); } else {
		 * reportPath=path +prop.getProperty(Constantes.REPORT_VENTAS_GENERAL_PROPS); }
		 * byte[] reporte= null; log.info("=======> generando reporte" ); if(
		 * formato.equalsIgnoreCase( "XLS" ) ){ reporte=
		 * ReportGenerator.generateReportExcel(mockJdbcConnection(), reportPath ,
		 * parameters ); } else { reporte=
		 * ReportGenerator.generateReportPdf(mockJdbcConnection(), reportPath ,
		 * parameters ); } log.info("=======> reporte generado " + reporte );
		 * FileOutputStream fileOuputStream = new
		 * FileOutputStream("C:\\relge/report/reporteXX.pdf");
		 * fileOuputStream.write(reporte); fileOuputStream.close(); } catch (Exception
		 * e) { e.printStackTrace(); }
		 */
	}
	...
	/**
	 * Metodo que carga genera el reporte recibiendo la conexion de base de datos,
	 * la logica de datos se maneja en el reporte
	 * 
	...
	public ByteArrayOutputStream generateReportWithDbConnection() {
		try {
			ByteArrayOutputStream baos = new ByteArrayOutputStream();
			log.info("====================> INGRESA A generateReportWithDbConnection this.reportName  "
					+ this.reportName);
			log.info("====================> INGRESA A generateReportWithDbConnection this.geDB  "
					+ this.geDB.isClosed());
	...
	public ByteArrayOutputStream generateReportWithListDatasource() throws RelativeException {
		try {
			...
			if (this.dataSource != null && !this.dataSource.isEmpty()) {
				jasperPrint = JasperFillManager.fillReport(this.reportName, hm,
						new JRBeanArrayDataSource(this.dataSource.toArray()));
			} else {
	 ...
	 * @return reporte generado
	 */
	public static byte[] generateReportPdf(String reportFilePath, String xpathSource, byte[] xmlForDs,
	...
	public static byte[] generateReportPdf(Connection conn, String reportFilePath, Map<String, Object> parameters) {
		...
		try {
			// log.info( "++++++>> Ingresa en el generateReportPdf con conexion " + conn + "
			// path " + reportFilePath );
			// log.info( "++++++>> Ingresa en el generateReportPdf con para,etros " +
			// parameters );
	...
	@SuppressWarnings("rawtypes")
	public static byte[] generateReportPdf(List dataSource, String reportFilePath, Map<String, Object> parameters)
	...
	public ByteArrayOutputStream generateReportExcelWithListDatasource() throws RelativeException {
		try {
			...
			ByteArrayOutputStream baos = new ByteArrayOutputStream();
			JasperPrint jasperPrint = JasperFillManager.fillReport(this.reportName, hm,
					new JRBeanArrayDataSource(this.dataSource.toArray()));
			JRXlsExporter exporter = new JRXlsExporter();
			exporter.setExporterInput(new SimpleExporterInput(jasperPrint));
			exporter.setExporterOutput(new SimpleOutputStreamExporterOutput(baos));
